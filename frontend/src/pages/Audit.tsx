...
export function Audit() {
  ...
  const onApply = (e: React.FormEvent) => {
    ...
    if (actor.trim()) params.actor_username = actor.trim();
  ...
  return (
    <div>
      ...
      <form onSubmit={onApply} style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr 120px auto', gap: 'var(--sp-3)', alignItems: 'end' }}>
          <FormField label="Action">
            {(s) => <input style={s} value={action} onChange={(e) => setAction(e.target.value)} placeholder="user.login" />}
          </FormField>
          <FormField label="Resource Type">
            {(s) => <input style={s} value={resourceType} onChange={(e) => setResourceType(e.target.value)} placeholder="device" />}
          </FormField>
          <FormField label="Actor Username">
            {(s) => <input style={s} value={actor} onChange={(e) => setActor(e.target.value)} placeholder="alice" />}
          </FormField>
