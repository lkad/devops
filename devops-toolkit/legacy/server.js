...
const metricsManager = require('./metrics_manager');
const alertNotificationManager = require('./alerts_notification_manager');
const physicalHostManager = new PhysicalHostManager({ poolSize: 10, heartbeatInterval: 30000 });
const networkDiscovery = new NetworkDiscovery({ network: '172.30.30.0/24', timeout: 2000 });
...
async function handleRequest(req, res) {
  ...
  if (pathname.startsWith('/api/')) {
    ...
    if (method === 'GET' && pathname === '/api/devices') {
      ...
      // For physical hosts, merge real-time status from PhysicalHostManager
      devices = devices.map(device => {
        if (device.type === 'physical_host') {
          const host = physicalHostManager.getHost(device.id);
          if (host) {
            // Map PhysicalHostManager state to DeviceManager status
            device.status = host.state === 'online' ? 'active' : 'inactive';
            device.metrics = host.metrics; // real-time metrics
            device.lastHeartbeat = host.lastHeartbeat;
          }
        }
        return device;
      });
    ...
    if (method === 'GET' && pathname.startsWith('/api/devices/')) {
      ...
      const device = deviceManager.getDevice(id);
      if (device) {
        // For physical hosts, merge real-time data from PhysicalHostManager
        if (device.type === 'physical_host') {
          const host = physicalHostManager.getHost(id);
          if (host) {
            device.status = host.state === 'online' ? 'active' : 'inactive';
            device.metrics = host.metrics;
            device.lastHeartbeat = host.lastHeartbeat;
            device.connectionInfo = { ip: host.ip, port: host.port, username: host.username };
          }
        }
    ...
    // ============ Physical Hosts API Routes ============

    // GET /api/physical-hosts - List all physical hosts
    if (method === 'GET' && pathname === '/api/physical-hosts') {
      const hosts = physicalHostManager.getAllHosts();
      const summary = physicalHostManager.getSummary();
      sendJSON(res, 200, { success: true, hosts, summary });
      return;
    }

    // POST /api/physical-hosts - Register a physical host
    if (method === 'POST' && pathname === '/api/physical-hosts') {
      try {
        const body = await parseBody(req);
        const { id, hostname, ip, port = 22, username, authMethod = 'password', credentials = {} } = body;
        if (!id || !hostname || !ip) {
          sendJSON(res, 400, { success: false, error: 'Missing required fields: id, hostname, ip' });
          return;
        }
        // Register in PhysicalHostManager
        const host = physicalHostManager.registerHost({ id, hostname, ip, port, username, authMethod, credentials });
        // Also register as a device in DeviceManager so it appears in device list
        deviceManager.registerDevice({
          id: id,
          name: hostname,
          type: 'physical_host',
          status: 'pending',
          labels: [{ env: 'monitored' }],
          business_unit: body.business_unit || null
        });
        sendJSON(res, 201, { success: true, host });
      } catch (err) {
    ...
    // GET /api/physical-hosts/:id - Get physical host details
    if (method === 'GET' && pathname.startsWith('/api/physical-hosts/')) {
      const id = pathname.split('/')[3];
      const host = physicalHostManager.getHost(id);
      if (host) {
        sendJSON(res, 200, { success: true, host });
      } else {
        sendJSON(res, 404, { success: false, error: 'Host not found' });
      }
      return;
    }

    // DELETE /api/physical-hosts/:id - Remove physical host
    if (method === 'DELETE' && pathname.startsWith('/api/physical-hosts/')) {
      const id = pathname.split('/')[3];
      const result = physicalHostManager.removeHost(id);
      // Also remove from DeviceManager
      deviceManager.removeDevice(id);
      sendJSON(res, result ? 200 : 404, { success: result });
      return;
    }

    // POST /api/physical-hosts/:id/collect-metrics - Collect metrics from host
    if (method === 'POST' && pathname.match(/^\/api\/physical-hosts\/[^/]+\/collect-metrics$/)) {
      const id = pathname.split('/')[3];
      try {
        const metrics = await physicalHostManager.collectMetrics(id);
        sendJSON(res, 200, { success: true, metrics });
      } catch (err) {
        sendJSON(res, 500, { success: false, error: err.message });
      }
    ...
    // POST /api/physical-hosts/:id/heartbeat - Trigger heartbeat check
    if (method === 'POST' && pathname.match(/^\/api\/physical-hosts\/[^/]+\/heartbeat$/)) {
      const id = pathname.split('/')[3];
      try {
        await physicalHostManager._doHeartbeat(id);
        const host = physicalHostManager.getHost(id);
        sendJSON(res, 200, { success: true, host });
      } catch (err) {
        sendJSON(res, 500, { success: false, error: err.message });
    ...
    // GET /api/physical-hosts/summary - Get host summary
    if (method === 'GET' && pathname === '/api/physical-hosts/summary') {
      const summary = physicalHostManager.getSummary();
      sendJSON(res, 200, { success: true, summary });
      return;
    }

    // POST /api/telegraf/receive - Receive metrics pushed from Telegraf agent
    // Telegraf agent on each host pushes metrics every 10s via HTTP POST
    if (method === 'POST' && pathname === '/api/telegraf/receive') {
      try {
        // Auth check
        const secret = req.headers['x-telegraf-key'] || req.headers['x-telegraf-key'.toLowerCase()];
        if (process.env.TELEGRAF_SHARED_SECRET && secret !== process.env.TELEGRAF_SHARED_SECRET) {
        ...
        if (!host_id || !metrics) {
          sendJSON(res, 400, { success: false, error: 'Missing required fields: host_id, metrics' });
          return;
        }

        const result = physicalHostManager.receiveAgentMetrics(host_id, metrics);
        if (result.success) {
          sendJSON(res, 200, result);
        } else {
          sendJSON(res, 404, result);
    ...
    // POST /api/prometheus/receive - Receive metrics from Prometheus remote_write
    // Prometheus server pushes metrics via remote_write API
    if (method === 'POST' && pathname === '/api/prometheus/receive') {
      try {
        // Auth check
        const auth = req.headers['authorization'] || '';
        const token = auth.startsWith('Bearer ') ? auth.slice(7) : auth;
        if (process.env.PROMETHEUS_SHARED_SECRET && token !== process.env.PROMETHEUS_SHARED_SECRET) {
          sendJSON(res, 401, { success: false, error: 'Unauthorized' });
          return;
        }

        const body = await parseBody(req);
        const result = physicalHostManager.receivePrometheusMetrics(body);
        sendJSON(res, 200, result);
      } catch (err) {
        sendJSON(res, 500, { success: false, error: err.message });
      }
    ...
    // GET /metrics - Prometheus exporter format
    if (method === 'GET' && pathname === '/metrics') {
      try {
        const metricsData = physicalHostManager.getPrometheusMetrics();
    ...
    // GET /api/discovery/status - Get discovery scan status
    if (method === 'GET' && pathname === '/api/discovery/status') {
      const status = networkDiscovery.getStatus();
    ...
    // POST /api/discovery/register - Register discovered devices
    if (method === 'POST' && pathname === '/api/discovery/register') {
      try {
        const body = await parseBody(req);
        const devices = networkDiscovery.getDiscoveredDevices();

        if (devices.length === 0) {
          sendJSON(res, 400, { success: false, error: 'No devices discovered. Run scan first.' });
          return;
        }
        ...
        for (const device of devices) {
          try {
            ...
            const existing = deviceManager.getDevice(device.id);
            if (existing) {
              failed.push({ device, error: 'Already registered' });
              continue;
            }

            // Register via DeviceManager
            const registeredDevice = deviceManager.registerDevice({
              id: device.id,
              type: device.type,
              name: device.name,
            ...
            metricsManager.recordDeviceEvent('discovered');
            wsManager.broadcastDeviceEvent({ type: 'discovered', device: registeredDevice });
          } catch (err) {
            failed.push({ device, error: err.message });
          }
        }
