...
// Dev-default secret constants live in
// internal/config/secrets_block.go. main.go references
// them as config.DevDefault*; production startup still
// refuses to start when these are in use (see
...
func registerAuthRoutes(r *gin.Engine, cfg *config.Config, log *logger.Logger) (*auth.Signer, error) {
	...
	secret := config.EnvOrWarn("APP_JWT_SECRET", config.DevDefaultJWTSecret)
	if secret == config.DevDefaultJWTSecret && cfg.App.Env == "production" {
...
func registerK8sClusterRoutes(v1 *gin.RouterGroup, db *gorm.DB, log *logger.Logger, perms func(rbacpkg.Permission) gin.HandlerFunc, auditSvc *audit.Service) *k8s.Service {
	...
	key := []byte(config.EnvOrWarn("K8S_CRYPTO_KEY", config.DevDefaultK8SCryptoKey))
	if len(key) != 32 {
